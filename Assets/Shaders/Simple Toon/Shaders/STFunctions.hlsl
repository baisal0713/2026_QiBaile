...
float invLerp (float from, float to, float value) {
	return invLerp(from, to, value, 0.0);
}

float invLerp01 (float from, float to, float value) {
	return invLerp01(from, to, value, 0.0);
}

float wght_invLerp (float from, float to, float value, bool invert) {
	float val = (value - from) / (to - from);
...
float smoothstepSafe (float from, float to, float value, float equal) {
	float val = smoothstep(from, to, value);
	return from == to ? equal : val;
}

float wght_smoothstep (float from, float to, float value, bool invert) {
	float val = smoothstep(from, to, value);
